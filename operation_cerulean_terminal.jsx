...
export default function TerminalApp() {
  ...
  const BOOT_SEQUENCE = [
    ...
    " ",
    "> SYSTEM ENCRYPTED. WHAT IS THE TRUE NAME OF UNKNOWN SUBJECT ONE?"
  ];
  ...
  const handleCommand = (e) => {
    ...
    // TIER 1: The Master Password (Strictly Case Sensitive)
    if (stage === 0) {
    ...
    // TIER 4 & 5: Payload Reversal & Final Reveal
    else if (stage === 3) {
      if (command === "2937") {
        setLocked(true);
        setHistory((prev) => [...prev, "PAYLOAD ACCEPTED. DECRYPTING MASTER FILE..."]);
        
        // The Suspense Trap (Fake Loading Sequence)
        ...
        // The Glitch Pause (Makes him sweat)
        ...
        setTimeout(() => {
          setHistory(prev => [
            ...
            " ",
            "YOUR ASSET IS SECURED IN: [PLACEHOLDER: ENTER EXACT PHYSICAL LOCATION HERE]",
            "=========================================",
            "HAPPY BIRTHDAY."
          ]);
        }, 11000);
  ...
  // The Sadistic No-Backspace Rule
      ...
      <div className="min-h-screen bg-black text-green-500 p-4 sm:p-8 font-mono text-sm sm:text-lg flex flex-col crt-effect relative">
        ...
        <div className="flex-1 overflow-y-auto mb-4 z-10 break-words whitespace-pre-wrap">
          {history.map((line, index) => (
            <div key={index} className="min-h-[1.5rem] mt-1 text-shadow-glow tracking-wide">
              {line}
            </div>
          ))}
          <div ref={bottomRef} />
        ...
        <form onSubmit={handleCommand} className="flex items-center z-10 border-t border-green-900 pt-4">
          ...
          <input
            ...
            disabled={locked}
            autoFocus
            className="bg-transparent border-none outline-none flex-1 text-green-400 font-bold disabled:opacity-50 tracking-widest uppercase-placeholder"
            spellCheck="false"
            autoComplete="off"
          />
        </form>
